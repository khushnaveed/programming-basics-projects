...
let musician = album.credits[0];
console.log(`${musician.name}: ${musician.instruments}`);
...
for (let i = 0; i < album.tracklist.length; i++) {
  let titles = album.tracklist[i];
  console.log(i+1 + '. ' + titles.title);  
}
...
for (let i = 0; i < album.credits.length; i++) {
  let band = album.credits[i];
  console.log(`${band.name}: ${band.instruments}`);  
}
