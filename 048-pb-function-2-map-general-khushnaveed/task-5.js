...
/* const productDescriptions = products.map((product) => {
  product.description = [product.color[0].toUpperCase() + product.color.slice(1), product.type, product.price];
  return product.description;
}); */

const productDescriptions = products.map((product) => {
  return `${product.color[0].toUpperCase()}${product.color.slice(1)}  ${product.type}, ${product.price}` ;
